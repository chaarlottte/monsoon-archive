...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        if (entity != null && this.mc.theWorld != null)
        {
            ...
            double blockReachDist = (double)this.mc.playerController.getBlockReachDistance();
            this.mc.objectMouseOver = entity.rayTrace(blockReachDist, partialTicks);
            double dist = blockReachDist;
            Vec3 vec3 = entity.getPositionEyes(partialTicks);
            boolean flag = false;

            EventMouseOver event = new EventMouseOver(3.0D, 0);
            Wrapper.getMonsoon().getBus().post(event);

            if (this.mc.playerController.extendedReach()) {
                blockReachDist = 6.0D;
                dist = 6.0D;
            } else if (blockReachDist > 3.0D) {
                flag = true;
            }

            if(event.getReach() > 3.1)
                flag = true;

            if (this.mc.objectMouseOver != null) {
                dist = this.mc.objectMouseOver.hitVec.distanceTo(vec3);
            }

            Vec3 vec31 = entity.getLook(partialTicks);
            Vec3 vec32 = vec3.addVector(vec31.xCoord * blockReachDist, vec31.yCoord * blockReachDist, vec31.zCoord * blockReachDist);
            this.pointedEntity = null;
            Vec3 vec33 = null;
            float f = 1.0F;
            List<Entity> list = this.mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec31.xCoord * blockReachDist, vec31.yCoord * blockReachDist, vec31.zCoord * blockReachDist).expand((double)f, (double)f, (double)f), Predicates.and(EntitySelectors.NOT_SPECTATING, new Predicate<Entity>()
            {
            ...
            for (int j = 0; j < list.size(); ++j)
            {
                ...
                float f1 = entity1.getCollisionBorderSize() + ((entity instanceof EntityPlayer && !entity.isInvisible()) ? event.getExpand() : 0);
            ...
            if (this.pointedEntity != null && (d2 < dist || this.mc.objectMouseOver == null))
            {
