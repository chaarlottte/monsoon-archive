...
import wtf.monsoon.client.modules.combat.KeepSprint;
...
public abstract class EntityPlayer extends EntityLivingBase
{
                ...
                if (f > 0.0F || f1 > 0.0F)
                {
                    ...
                    if (flag2)
                    {
                        if (i > 0)
                        {
                            ...
                            if(!Wrapper.getMonsoon().moduleManager.getModule(KeepSprint.class).isEnabled()) {
                                this.motionX *= 0.6D;
                                this.motionZ *= 0.6D;
    ...
    @Identifier(identifier = "EnumChatVisibility")
    public static enum EnumChatVisibility
    {
        ...
        @Identifier(identifier = "get_chat_visiblity")
        public int getChatVisibility()
        {
            return this.chatVisibility;
        }

        @Identifier(identifier = "get_enum_chat_visibility")
        public static EntityPlayer.EnumChatVisibility getEnumChatVisibility(int id)
        {
            return ID_LOOKUP[id % ID_LOOKUP.length];
        }

        @Identifier(identifier = "get_resource_key")
    ...
    @Identifier(identifier = "EnumStatus")
    public static enum EnumStatus
    {
        @Identifier(identifier = "OK")
        OK,

        @Identifier(identifier = "NOT_POSSIBLE_HERE")
        NOT_POSSIBLE_HERE,

        @Identifier(identifier = "NOT_POSSIBLE_NOW")
        NOT_POSSIBLE_NOW,

        @Identifier(identifier = "TOO_FAR_AWAY")
        TOO_FAR_AWAY,

        @Identifier(identifier = "OTHER_PROBLEM")
        OTHER_PROBLEM,

        @Identifier(identifier = "NOT_SAFE")
        NOT_SAFE
    }
}
