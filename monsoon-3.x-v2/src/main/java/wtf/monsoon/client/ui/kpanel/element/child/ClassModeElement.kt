 ...
 * @since 21/02/2023
 */
class ClassModeElement(setting: Setting<ModuleMode<*>>, x: Float, y: Float, width: Float, height: Float) : SettingElement<ModuleMode<*>>(setting, x, y, width, height) {
    ...
    override fun draw(mouseX: Int, mouseY: Int, mouseDelta: Float) {
        ...
        ui.text(setting.getValue().name, x + width - ui.textWidth(setting.getValue().name, "regular", 14f) - 5, y + height / 2f + 1, "regular", 14f, Color.WHITE, Alignment.LEFT_MIDDLE)
