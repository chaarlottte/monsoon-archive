...
open class EventBus(private val config: Config = Config()) {
     ...
     * [Config.annotationRequired] is false, or they are annotated with [listener].
     *
     ...
     */
    open fun subscribe(subscriber: Any): Boolean = subscribers.add(subscriber).also {
     ...
     */
    open fun unsubscribe(subscriber: Any): Boolean = subscribers.remove(subscriber).also {
     ...
     */
    open fun post(event: Any): Boolean = listeners[event::class]?.post(event) ?: false
