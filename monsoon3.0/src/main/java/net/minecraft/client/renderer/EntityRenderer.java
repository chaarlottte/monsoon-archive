...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        if (entity != null && this.mc.theWorld != null)
        {
            ...
            double reach = reachModule.isEnabled() ? reachModule.blockReach : this.mc.playerController.getBlockReachDistance();
            this.mc.objectMouseOver = entity.rayTrace(reach, partialTicks);
            double blockOrSmth = reach;
            Vec3 vec3 = entity.getPositionEyes(partialTicks);
            boolean flag = false;

            double reachToFlag = reachModule.isEnabled() ? Math.round(reachModule.hitReach * 100.0) / 100.0 : 3.0D;
            ...
            //hitbox expand here
            float toExpand = 1.0F;
            List<Entity> list = this.mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec31.x * reach, vec31.y * reach, vec31.z * reach).expand(toExpand, toExpand, toExpand), Predicates.and(EntitySelectors.NOT_SPECTATING, Entity::canBeCollidedWith));
            double d2 = blockOrSmth;

            for (Entity entity1 : list) {
                float f1 = entity1.getCollisionBorderSize() + (hitboxModule.isEnabled() ? hitboxModule.expand.getValue()/10 : 0);
    ...
    {
        if(Wrapper.getModule(NoRender.class).isEnabled() && Wrapper.getModule(NoRender.class).hurtCam.getValue()) return;
    ...
    public void renderHand(float p_renderHand_1_, int p_renderHand_2_, boolean p_renderHand_3_, boolean p_renderHand_4_, boolean p_renderHand_5_)
    {
        if (!this.debugView)
        {
            ...
            Project.gluPerspective(event.getNewFOV(), (float)this.mc.displayWidth / (float)this.mc.displayHeight, 0.05F, this.farPlaneDistance * 2.0F);
            GlStateManager.matrixMode(5888);
            GlStateManager.loadIdentity();
    ...
    public void func_181560_a(float p_181560_1_, long p_181560_2_)
    {
        ...
        if (!this.mc.skipRenderWorld)
        {
            ...
            if (Wrapper.getModule(Blur.class) != null && Wrapper.getModule(Blur.class).isEnabled()) {
                if (!(!Wrapper.getModule(Blur.class).chat.getValue() && this.mc.currentScreen instanceof GuiChat)) {
                    BlurUtil.alternateBlur(0, -5, scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight() + 5, (int) ((2 * Wrapper.getModule(Blur.class).guiBlurStrength.getValue()) * mc.getBlurAnimation().getAnimationFactor()));
                }
            }

            if(BlurUtil.framebuffer != null) BlurUtil.framebuffer.unbindFramebufferTexture();

            RoundedUtils.gradient(0, 0, mc.displayWidth, mc.displayHeight, 0, (float) (0.5f * mc.getGradientAnimation().getAnimationFactor()),
            ...
            if (this.mc.currentScreen != null) {
                ...
                try
                {
                    ...
                    if(Screen.class.isAssignableFrom(mc.currentScreen.getClass())) {
                        Wrapper.getGlgui().render(mc.currentScreen);
    ...
    private void renderWorldDirections(float partialTicks)
    {
        if (this.mc.gameSettings.showDebugInfo && !this.mc.gameSettings.hideGUI && !this.mc.thePlayer.hasReducedDebug() && !this.mc.gameSettings.reducedDebugInfo)
        {
            ...
            RenderGlobal.drawSelectionBoundingBox(new AxisAlignedBB(0.0D, 0.0D, 0.0D, 0.005D, 1.0E-4D, 1.0E-4D), 255, 0, 0, 255);
            RenderGlobal.drawSelectionBoundingBox(new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0E-4D, 1.0E-4D, 0.005D), 0, 0, 255, 255);
            RenderGlobal.drawSelectionBoundingBox(new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0E-4D, 0.0033D, 1.0E-4D), 0, 255, 0, 255);
            GlStateManager.popMatrix();
                    ...
                    if (biomegenbase.canSpawnLightningBolt() || biomegenbase.getEnableSnow())
                    {
                        ...
                        if (k2 != l2)
                        {
                            ...
                            if (world.getWorldChunkManager().getTemperatureAtHeight(f1, j2) >= 0.15F)
                            {
                                ...
                                int k3 = j3 >> 16 & 65535;
                                int l3 = j3 & 65535;
                                worldrenderer.pos((double)l1 - d3 + 0.5D, (double)k2, (double)k1 - d4 + 0.5D).func_181673_a(0.0D, (double)k2 * 0.25D + d5).func_181666_a(1.0F, 1.0F, 1.0F, f3).func_181671_a(k3, l3).end();
                                worldrenderer.pos((double)l1 + d3 + 0.5D, (double)k2, (double)k1 + d4 + 0.5D).func_181673_a(1.0D, (double)k2 * 0.25D + d5).func_181666_a(1.0F, 1.0F, 1.0F, f3).func_181671_a(k3, l3).end();
                                worldrenderer.pos((double)l1 + d3 + 0.5D, (double)l2, (double)k1 + d4 + 0.5D).func_181673_a(1.0D, (double)l2 * 0.25D + d5).func_181666_a(1.0F, 1.0F, 1.0F, f3).func_181671_a(k3, l3).end();
                                worldrenderer.pos((double)l1 - d3 + 0.5D, (double)l2, (double)k1 - d4 + 0.5D).func_181673_a(0.0D, (double)l2 * 0.25D + d5).func_181666_a(1.0F, 1.0F, 1.0F, f3).func_181671_a(k3, l3).end();
                            }
                            else
                            {
                                if (j1 != 1)
                                ...
                                int j4 = i4 >> 16 & 65535;
                                int k4 = i4 & 65535;
                                worldrenderer.pos((double)l1 - d3 + 0.5D, (double)k2, (double)k1 - d4 + 0.5D).func_181673_a(0.0D + d9, (double)k2 * 0.25D + d8 + d10).func_181666_a(1.0F, 1.0F, 1.0F, f4).func_181671_a(j4, k4).end();
                                worldrenderer.pos((double)l1 + d3 + 0.5D, (double)k2, (double)k1 + d4 + 0.5D).func_181673_a(1.0D + d9, (double)k2 * 0.25D + d8 + d10).func_181666_a(1.0F, 1.0F, 1.0F, f4).func_181671_a(j4, k4).end();
                                worldrenderer.pos((double)l1 + d3 + 0.5D, (double)l2, (double)k1 + d4 + 0.5D).func_181673_a(1.0D + d9, (double)l2 * 0.25D + d8 + d10).func_181666_a(1.0F, 1.0F, 1.0F, f4).func_181671_a(j4, k4).end();
                                worldrenderer.pos((double)l1 - d3 + 0.5D, (double)l2, (double)k1 - d4 + 0.5D).func_181673_a(0.0D + d9, (double)l2 * 0.25D + d8 + d10).func_181666_a(1.0F, 1.0F, 1.0F, f4).func_181671_a(j4, k4).end();
                            }
                        }
                    }
                }
