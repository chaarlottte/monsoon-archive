    ...
    public void renderTileEntityAt(TileEntityEndPortal te, double x, double y, double z, float partialTicks, int destroyStage)
    {
        if (!Config.isShaders() || !ShadersRender.renderEndPortal(te, x, y, z, partialTicks, destroyStage, 0.75F))
        {
            ...
            for (int i = 0; i < 16; ++i)
            {
                ...
                worldrenderer.pos(x, y + (double)f3, z + 1.0D).func_181666_a(f11, f12, f13, 1.0F).end();
                worldrenderer.pos(x + 1.0D, y + (double)f3, z + 1.0D).func_181666_a(f11, f12, f13, 1.0F).end();
                worldrenderer.pos(x + 1.0D, y + (double)f3, z).func_181666_a(f11, f12, f13, 1.0F).end();
                tessellator.draw();
                GlStateManager.popMatrix();
