        ...
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, new Object[] {entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z)}))
        {
            ...
            try
            {
                ...
                if (entity.equals(Minecraft.getMinecraft().thePlayer)) {
                    ...
                    //f = this.interpolateRotation(entity.prevRenderYawOffset, event.getYaw(), partialTicks);
                    //f1 = this.interpolateRotation(entity.prevRotationYawHead, event.getYaw(), partialTicks);
                    //f2 = f1 - f;
                }
    ...
    protected boolean setScoreTeamColor(T entityLivingBaseIn)
    {
        ...
        if(Wrapper.getModule(ShaderESP.class).shouldRenderMinecraftOutlin()) {
            ...
            if (entityLivingBaseIn instanceof EntityLivingBase) {
                EntityLivingBase elb = (EntityLivingBase) entityLivingBaseIn;
                if(elb.hurtTime > 0) color = new Color(218,96,110);
            }
            // Color color = new Color(color1.getRGB());
            ...
            if (this.canRenderName(entity))
            {
                ...
                if (d0 < (double)(f * f))
                {
                    ...
                    if (entity.isSneaking())
                    {
                        ...
                        worldrenderer.pos((double)(-i - 1), -1.0D, 0.0D).func_181666_a(0.0F, 0.0F, 0.0F, 0.25F).end();
                        worldrenderer.pos((double)(-i - 1), 8.0D, 0.0D).func_181666_a(0.0F, 0.0F, 0.0F, 0.25F).end();
                        worldrenderer.pos((double)(i + 1), 8.0D, 0.0D).func_181666_a(0.0F, 0.0F, 0.0F, 0.25F).end();
                        worldrenderer.pos((double)(i + 1), -1.0D, 0.0D).func_181666_a(0.0F, 0.0F, 0.0F, 0.25F).end();
                        tessellator.draw();
                        GlStateManager.enableTexture2D();
