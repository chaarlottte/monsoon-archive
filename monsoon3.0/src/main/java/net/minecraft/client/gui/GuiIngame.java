...
public class GuiIngame extends Gui
{
    ...
    Animation openAnimation = new Animation(() -> 800f, false, () -> Easing.CUBIC_IN_OUT);
    public void renderGameOverlay(float partialTicks)
    {
        ...
        if (this.field_175195_w > 0)
        {
            ...
            if (i2 > 8)
            {
                ...
                this.getFontRenderer().drawString(this.subTitle, (float)(-this.getFontRenderer().getStringWidth(this.subTitle) / 2), 5.0F, 16777215 | j2, true);
                GlStateManager.popMatrix();
                GlStateManager.disableBlend();
                GlStateManager.popMatrix();
            }
        ...
        final float scaleFactor = 1.0f / (scaledresolution.getScaleFactor() * .5f);
        RenderUtil.scale(0,0,new float[]{scaleFactor,scaleFactor}, () -> {
            BlurUtil.blur_shader_2.bindFramebuffer(false);

            if(Wrapper.getModule(HUD.class).blur.getValue()) {
                BlurUtil.blur(Wrapper.getModule(HUD.class).blurStrength.getValue(), 1);
                KawesaUtil.preBlur();
                ...
                Wrapper.getMonsoon().getModuleManager().getModules().stream().filter(module -> module instanceof HUDModule).forEach(module -> {
                    ...
                    RenderUtil.scaleXY(hudModule.getX() + hudModule.getWidth() / 2f, hudModule.getY() + hudModule.getHeight() / 2f, module.getToggleHudAnimation(), hudModule::blur);
                });

                BlurUtil.postBlur(Wrapper.getModule(HUD.class).blurStrength.getValue(), 1);
            }

            mc.getFramebuffer().bindFramebuffer(false);
//        BloomUtil.bloom(20,2);
            ...
            Wrapper.getMonsoon().getModuleManager().getModules().stream().filter(module -> module instanceof HUDModule).forEach(module -> {
                ...
                RenderUtil.scaleXY(h.getX() + h.getWidth() / 2f, h.getY() + h.getHeight() / 2f, module.getToggleHudAnimation(), () -> {
                    h.render();
                    RoundedUtils.outline(h.getX(), h.getY(), h.getWidth(), h.getHeight(), 10F, -2f + (float) h.getLinearHoverAnimation().getAnimationFactor()*3, Color.WHITE);
                });
            });

            Wrapper.getNotifManager().render();
            Wrapper.getFont().drawString("cock", -400, -400, Color.WHITE, false);
        });
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks) {
        EventRenderHotbar.Pre pre = new EventRenderHotbar.Pre(new ScaledResolution(mc));
    ...
    protected boolean showCrosshair() {
        // hide crosshair whilst in third person lel
        if (mc.gameSettings.thirdPersonView > 0 || Wrapper.getModule(CrosshairCustomizer.class).isEnabled()) {
            return false;
        }
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_) {
        ...
        for (Score score1 : collection) {
            ...
            try {
                if(s1.split(": ")[0].equals("Map")) {
                   for(String map : Wrapper.getModule(AutoHypixel.class).getBlacklistedMaps()) {
                       if(s1.toLowerCase().contains(map.toLowerCase())) {
    ...
    private void renderPumpkinOverlay(ScaledResolution p_180476_1_)
    {
        ...
        worldrenderer.pos(0.0D, (double)p_180476_1_.getScaledHeight(), -90.0D).func_181673_a(0.0D, 1.0D).end();
        worldrenderer.pos((double)p_180476_1_.getScaledWidth(), (double)p_180476_1_.getScaledHeight(), -90.0D).func_181673_a(1.0D, 1.0D).end();
        worldrenderer.pos((double)p_180476_1_.getScaledWidth(), 0.0D, -90.0D).func_181673_a(1.0D, 0.0D).end();
        worldrenderer.pos(0.0D, 0.0D, -90.0D).func_181673_a(0.0D, 0.0D).end();
        tessellator.draw();
    ...
    private void renderVignette(float p_180480_1_, ScaledResolution p_180480_2_)
    {
        ...
        else
        {
            ...
            WorldRenderer worldrenderer = tessellator.getWorldRenderer();
            worldrenderer.begin(7, DefaultVertexFormats.field_181707_g);
            worldrenderer.pos(0.0D, (double)p_180480_2_.getScaledHeight(), -90.0D).func_181673_a(0.0D, 1.0D).end();
            worldrenderer.pos((double)p_180480_2_.getScaledWidth(), (double)p_180480_2_.getScaledHeight(), -90.0D).func_181673_a(1.0D, 1.0D).end();
            worldrenderer.pos((double)p_180480_2_.getScaledWidth(), 0.0D, -90.0D).func_181673_a(1.0D, 0.0D).end();
            worldrenderer.pos(0.0D, 0.0D, -90.0D).func_181673_a(0.0D, 0.0D).end();
            tessellator.draw();
            GlStateManager.depthMask(true);
            GlStateManager.enableDepth();
    ...
    private void func_180474_b(float p_180474_1_, ScaledResolution p_180474_2_)
    {
        ...
        WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        worldrenderer.begin(7, DefaultVertexFormats.field_181707_g);
        worldrenderer.pos(0.0D, (double)p_180474_2_.getScaledHeight(), -90.0D).func_181673_a((double)f, (double)f3).end();
        worldrenderer.pos((double)p_180474_2_.getScaledWidth(), (double)p_180474_2_.getScaledHeight(), -90.0D).func_181673_a((double)f2, (double)f3).end();
        worldrenderer.pos((double)p_180474_2_.getScaledWidth(), 0.0D, -90.0D).func_181673_a((double)f2, (double)f1).end();
        worldrenderer.pos(0.0D, 0.0D, -90.0D).func_181673_a((double)f, (double)f1).end();
        tessellator.draw();
        GlStateManager.depthMask(true);
        GlStateManager.enableDepth();
        GlStateManager.enableAlpha();
