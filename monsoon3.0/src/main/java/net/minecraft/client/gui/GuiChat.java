...
public class GuiChat extends GuiScreen
{
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException
    {
        ...
        Wrapper.getMonsoon().getModuleManager().getModules().stream().filter(m -> m instanceof HUDModule && m.isEnabled()).forEach(m -> {
            if(mouseButton == 0 && ((HUDModule) m).hovered(mouseX/scaleFactor, mouseY/scaleFactor)) {
                dragObj = (HUDModule) m;
            }
        });
    ...
    void drag(HUDModule module, float mx, float my) {
        ...
        if(module.hovered(mx,my) && Mouse.isButtonDown(0)) {
            if (!this.drag) {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Wrapper.getMonsoon().getModuleManager().getModules().stream().filter(m -> m instanceof HUDModule).forEach(m -> {
            ...
            hm.getHoverAnimation().setState(hm.hovered(mouseX / scaleFactor, mouseY / scaleFactor));
            hm.getLinearHoverAnimation().setState(hm.hovered(mouseX / scaleFactor, mouseY / scaleFactor));
        });
        ...
        if(dragObj != null)
            drag(dragObj, mouseX/scaleFactor, mouseY/scaleFactor);
